...
use crate::pam::{AuthStatus, PamSession, PamStringItem};
use crate::OlmanError::*;
use nix::unistd::{initgroups, isatty, setgid, setuid, Gid, Pid, Uid};
use regex::Regex;
use slog::Drain;
...
lazy_static! {
    ...
    static ref AUTH_STORE: Mutex<Credentials> = Mutex::new(Credentials::new());
...
fn session_utmpx() -> Result<UTMPXEntry, OlmanError> {
    match *tty::STDIN_TTYNAME {
        Some(ref _ttyname) => {
            ...
            utmp.iter()
                .find(|entry| {
                    Pid::from_raw(entry.pid()) == Pid::this()
                        && entry.id() != ""
...
fn update_utmpx<S>(name: S, line: S, host: S, utmp: &mut UTMPXEntry)
where
    S: Into<String>,
{
    let h = host.into();
    let hostname = if h != "" { h } else { utmp.host().into() };
    ...
    utmp.0.ut_session = nix::sys::termios::tcgetsid(std::io::stdin().as_raw_fd())
        .unwrap_or(Pid::from_raw(-1))
        .into();
    utmp.0.ut_tv = libc::__timeval {
        tv_sec: epoch_time.as_secs() as i32,
        tv_usec: (epoch_time.as_secs() * 1000 + epoch_time.subsec_nanos() as u64 / 1_000_000)
            as i32,
    };
    // TODO: log ut_addr_v6
}
...
fn olman() -> Result<(), OlmanError> {
    ...
    let olman_toml: Config =
        toml::from_str(&std::fs::read_to_string("/etc/olman.toml").unwrap_or("".into()))
            .unwrap_or_default();

    let _log_root = slog::Logger::root(
        slog_syslog::unix_3164(slog_syslog::Facility::LOG_AUTHPRIV)
            .unwrap()
            .ignore_res(),
        o!(),
    );
    ...
    let hostname = utmpx_entry.as_ref().map(|e| e.host()).unwrap_or("");
    ...
    let _ = env::var("LANG").and_then(|v| Ok(new_env.insert("LANG".into(), v)));
    // Add the timezone environment variable so that time functions work
    // correctly
    let _ = env::var("TZ").and_then(|v| Ok(new_env.insert("TZ".into(), v)));
    // Add the clock frequency so that profiling commands work correctly
    let _ = env::var("HZ").and_then(|v| Ok(new_env.insert("HZ".into(), v)));
    ...
    let origin = if hostname != "" {
        format!(" on '{:.100}' from '{:.200}'", current_tty, hostname)
    } else {
    ...
    let frontend_user = users::get_user_by_name(&frontend_username)
        .ok_or(OlmanError::NoUser(frontend_username.clone()))?
        .uid();
    let frontend_group = users::get_group_by_name(&frontend_groupname)
        .ok_or(OlmanError::NoGroup(frontend_groupname.clone()))?
        .gid();
    ...
    loop {
        ...
        let mut pam_session = PamSession::new("login", Some(pam::pam_conversation))?;
        pam_session.set_string_item(PamStringItem::RequestingHost, hostname)?;
        pam_session.set_string_item(PamStringItem::TTY, current_tty.clone())?;
        pam_session.set_fail_delay(1000000 * delay)?;

        let mut auth_store = AUTH_STORE.lock().unwrap();
        auth_store.set_credentials(|| {
            let frontend = unsafe {
                Command::new(frontend_cmd.clone())
                    ...
                    .output()
                    .map_err(|e| OlmanError::SpawnError(e.to_string().into()))?
            };
            let username: String = USER_RE
                .captures(&String::from_utf8_lossy(&frontend.stdout))
                .and_then(|c| c.get(1).map(|m| m.as_str().into()))
                .unwrap_or("".into());
            ...
            pam_session.set_string_item(PamStringItem::User, username.clone())?;
        ...
        match pam_session.authenticate() {
            AuthStatus::Authenticated => {
                match pam_session.validate_account() {
                    Err(OlmanError::AuthTokenExpired) => pam_session.change_expired_token()?,
                    Err(e) => return Err(e),
                    _ => {}
                };
                ...
                let username = pam_session
                    .get_string_item(PamStringItem::User)
                ...
                setgid(Gid::from_raw(passwd.gid)).map_err(|_e| {
                    OlmanError::BadGroupID(Gid::from_raw(passwd.gid), passwd.name.clone())
                })?;
                initgroups(
                    &std::ffi::CString::new(passwd.name.clone()).unwrap(),
                    Gid::from_raw(passwd.gid),
                )
                .map_err(|_e| OlmanError::InitgroupsFailed(passwd.name.clone()))?;
                ...
                .map_err(|e| OlmanError::TtyOwnership(e.to_string().into(), passwd.name.clone()))?;
                ...
                let mut session_proc = match session.command.clone() {
                    ...
                    None => {
                        session_shell = session.shell.clone().unwrap_or(passwd.shell);
                        let mut c = Command::new(session_shell.clone());
                        c.args(&["-l"]);
                        if session.graphical {
                            c.args(&[&command_flag, &olman_toml.graphical.command]);
                        }
                        c
                    }
                };
                ...
                if let Ok(mut utmpx) = session_utmpx() {
                    update_utmpx(username, current_tty.into(), hostname.into(), &mut utmpx);
                    let _ = SYSTEM_UTMPX.lock().unwrap().write_entry(utmpx);
                }
                ...
                unsafe {
                    session_proc
                                ...
                                Some(command) => {
                                    ...
                                    let argv = &[
                                        std::ffi::CString::new(format!(
                                            "-{}",
                                            std::path::Path::new(&session_shell.clone())
                                                .file_name()
                                                .unwrap()
                                                .to_string_lossy()
                                        ))
                                        .unwrap(),
                                        std::ffi::CString::new(command_flag.clone()).unwrap(),
                                        std::ffi::CString::new(cmd).unwrap(),
                                    ];

                                    nix::unistd::execv(
                                        &std::ffi::CString::new(session_shell.clone()).unwrap(),
                                        argv,
                                    )
                                    .map(|_| ())
                                ...
                                None => {
                                    if session.graphical {
                                        let argv = &[
                                            ...
                                            .unwrap(),
                                            std::ffi::CString::new(command_flag.clone()).unwrap(),
                                            std::ffi::CString::new(graphical_command.clone())
                                                .unwrap(),
                                        ];
                                        ...
                                        let argv = &[std::ffi::CString::new(format!(
                                            "-{}",
                                            std::path::Path::new(&session_shell.clone())
                                                .file_name()
                                                .unwrap()
                        ...
                        .status()
                        .map_err(|e| OlmanError::ShellError(e.to_string().into()))?;
                }

                return Ok(());
            }
            AuthStatus::MaxTriesExceeded => {
                return Err(OlmanError::TooManyLogins(
                    login_attempts,
                    origin,
                    current_username,
                ))
...
fn main() {
    ::std::process::exit(match olman() {
        ...
        Err(NoUser(u)) => {
            eprintln!(
                "{}: Unable to find \"{}\" in the user database.",
                *PROG_NAME, u
            );
            // TODO: log here
            1
        ...
        Err(NoGroup(g)) => {
            eprintln!(
                "{}: Unable to find \"{}\" in the group database.",
                *PROG_NAME, g
            );
            // TODO: log here
            1
        ...
        Err(SpawnError(e)) => {
            eprintln!("{}: Error spawning frontend process: {}", *PROG_NAME, e);
            // TODO: log here
            1
        }
        Err(PamInitError) => {
            eprintln!("{}: PAM initialization failure, aborting", *PROG_NAME);
            // TODO: log here
            99
        }
        Err(PamError(e)) => {
            eprintln!("{}: PAM error: {}", *PROG_NAME, e);
            // TODO: log here
            1
        }
        Err(PamAbort) => {
        ...
        Err(AuthTokenExpired) => {
            ...
            eprintln!("{}: Password/authentication token expired", *PROG_NAME);
            0
        }
        Err(CannotFindUser(u)) => {
            eprintln!("{}: Cannot find user ({})", *PROG_NAME, u);
            // TODO: log here
            1
        }
        Err(BadGroupID(i, u)) => {
        ...
        Err(TtyOwnership(d, u)) => {
            eprintln!(
                "{}: Unable to change owner or mode of tty stdin: {}",
                *PROG_NAME, d
            );
            // TODO: log here
            let _u = u;
        ...
        Err(ShellError(e)) => {
            eprintln!("{}: Error spawning shell: {}", *PROG_NAME, e);
            // TODO: log here
            1
        }
    });
